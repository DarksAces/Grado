// Ejemplo Redactor 2.cpp : Este archivo contiene la función "main". La ejecución del programa comienza y termina ahí.
//
...
int main() {
    ...
    while (hambrePers1 > 0 && hambrePers2 > 0) {
        ...
        personaCome(hambrePers1, comidaPersona1);
        if (hambrePers1 <= 0) {
        ...
        personaCome(hambrePers2, comidaPersona2);
        if (hambrePers2 <= 0) {
...
void personaCome(int& hambrePersona, int comidaPersona) {
    hambrePersona -= comidaPersona;
    if (hambrePersona < 0)
        hambrePersona = 0;
    cout << "¡La persona come " << comidaPersona << " hamburguesas!" << endl;
    cout << "Hambre restante: " << hambrePersona << endl;
    cout << endl;
}
